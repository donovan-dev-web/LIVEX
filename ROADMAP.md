...
| **U8** — Tests & persistance (V0.1 → V1) | ph7 (`SYNE-070…073`) + ph11 (`SYNE-110…113`) + `SYNE-120…122` + `SYNE-130…131` | ph8 (`ECHOS-080…085`) + ph9 (`ECHOS-090…092`) | **Jalon U8 complet** — SYNE-070→073, 110→113, 120→122, 130→131 ; ECHOS-080→085, 090→092 | SYNE : cycle ressources/constructions/saisons/territoires ; livres versionnés/persistés ; baselines de calibration provisoires tracées ADR-014 ; migration morphologie ; rapport ECHOS post-run déterministe et lecture seule. Persistance bit-à-bit et contrôle :5181 validés. ECHOS : tests/goldens (96,01 % mesuré), intégration CI sur le vrai WebSocket SYNE avec persistance/API et rapport calibration. PRISM runtime est une phase ultérieure ; ECHOS-091 est validé ici au niveau contrat d'intégration, pas comme rendu PRISM. |
