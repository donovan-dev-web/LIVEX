...
| SYNE-071 | Constructions (obstacles statiques) — **LIVRÉ (PR, jalon U8)** | `type/feature`, `component/syne` | P1 | décision n°20, `SYSTEMS_SPEC.md` §3.14, §6.4 | Construction = obstacle statique de la grille ; modification d'environnement tracée — ✓ `world.obstacles` vivant + `world.obstacleLayout[]` (initial), `AddObstacle` validé (bornes/id unique) + révision, constructions tracées `PlaceConstruction`/`RemoveConstruction` (drainées en `world.construction_placed`/`world.construction_removed`), grille A\* re-rasterisable via `Refresh()` (purge cache, no-op sinon), snapshot `obstacles[]` (API_CONTRACTS §2.1), bornes `world.obstacleLayout[]` validées ; engineVersion **0.8.0**, checksums dorés inchangés (0 tirage PRNG) ; **aucune mécanique agentique** (mathématiques ouvertes, décision n°20) |
| SYNE-072 | Saisons (cycle environnemental) — **LIVRÉ (PR, jalon U8)** | `type/feature`, `component/syne` | P2 | SYNE-070, `SYSTEMS_SPEC.md` §3 | Variations périodiques appliquées ; déterminisme conservé — ✓ `world.seasons` (objet : `enabled` défaut `false`, `seasonLengthTicks` 360, `initialSeason` spring, `cycle[]` 4 définitions × facteurs/resource — l'ancien booléen mort est remplacé), saison = **fonction pure du tick** `(initialIndex + tick / length) mod 4` (0 tirage PRNG), `SeasonFactors` × régénération/dégradation (`ResourceStocks.ApplyLifecycle`), basculement tracé + drainé `world.season_changed` (au tick exact, avant le snapshot), snapshot `season`/`seasonIndex` (additif), validation §6 (`SeasonTests` + `ObservabilitySeasonTests`, 19 tests) ; engineVersion **0.9.0**, checksums dorés inchangés (saisons désactivées par défaut — scénario de référence intact) |
| SYNE-073 | Territoire (décision n°21) — **LIVRÉ (PR, jalon U8)** | `type/feature`, `component/syne` | P1 | décision n°21, `SYSTEMS_SPEC.md` §3.10 | Territoire = zone délimitée par la présence d'entités autour d'un point de survie ; observation V0.1 — ✓ `world.territories` (bloc `{enabled, zones[]}` : disques {id, centerX, centerY, radius} « points de survie »), posées à l'init (`ApplyConfiguredLayout`), **appartenance = fonction pure des positions** (0 tirage PRNG, suivie en fin de tick par `SimulationLoop.TrackTerritoryMembership`, ordre déterministe zone/id, sorties avant entrées), tracée + drainée `world.territory_membership_changed` (agentId = entité, targetId = zone, value {kind}), snapshot `territories[]` {id, x, y, radius, memberCount, members[]} (additif, désactivé ⇒ `[]`), validations §6 (± layout) ; engineVersion **0.10.0**, checksums dorés inchangés (suivi désactivé par défaut — scénario de référence intact) ; les sources spatiales de ressources du territoire restent hors V0.1 |
...
| SYNE-113 | Serveur de contrôle HTTP :5181 (start/pause/resume/reset) — **LIVRÉ (PR SYNE, U8)** | `type/feature`, `component/syne` | P0 | ADR-002, `API_CONTRACTS.md` §2.4, `COMMUNICATION_PROTOCOL.md` | ✓ Contrôle **non intrusif** du moteur SYNE exposé sur HTTP :5181 (`Simulation.Console/Control/` : `ControlServer` HttpListener BCL + `SimulationController` ; contrats API_CONTRACTS §3 : `start {seed?, config?}` / `pause` / `resume` / `reset {seed?, runId?}` + `GET /status`, machine à états idle→running⇋paused→finished, mode CLI `--serve`, défaut 5181) ; 0 tirage PRNG ajouté ⇒ trajectoire bit-à-bit identique au run ininterrompu (test `ControlledRun_MatchesBatchRun_TickForTick`) ; finalise **ECHOS-085** — consommé tel quel par le `ControlClient` ECHOS (interop testé) |
