...
| Observabilité (SYNE-080) | format camelCase des messages (snapshot/event), épinglage et déterminisme d'émission, contrat `decision_made` + `action_completed` (avec réserves) + `message_sent`/`message_received` (contrats de livraison/réception) + `agent_died` ({cause, species}) + `agent_spawned` (parentage), `resources` peuplées (4 types dep. SYNE-070), **engineVersion** ; **tests de fil WebSocket réels** (`Simulation.Console.Tests`) |
