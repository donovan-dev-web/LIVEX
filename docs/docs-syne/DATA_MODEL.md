...
- **Zones de territoire** : `Territory {Id, Center, Radius}` (SYNE-073, engineVersion 0.10.0) — disques « points de survie » configurés par layout `world.territories.zones[]` (CONFIGURATION §6.10) ; le **territoire effectif** = présence des entités dans le disque (`distance ≤ radius`), revue en fin de tick (pure des positions, 0 tirage PRNG — DETERMINISM.md §3) ; appartenance tracée `TerritoryMembershipChange {Entered|Left}` (par zone/identifiant croissant, sorties avant entrées) + `MembersOf(zoneId)` ; champ snapshot `territories[]` `{id, x, y, radius, memberCount, members[]}` (additif, présent seulement si suivi actif), événement `world.territory_membership_changed` (API_CONTRACTS §2.2). Purement observationnel V0.1 — les **sources spatiales de ressources** restent V1 (sources par zone).
### 2.1 Livres (SYNE-121, engineVersion 0.11.0)

`Book {Id, AuthorId, Title, Content, WrittenTick, Readers[]}` matérialise un savoir figé au tick de rédaction. L’écriture est explicite, débite `world.books.writeCostEnergy` à l’auteur et produit `world.book_written`; la lecture ajoute un lecteur distinct (ordre de première lecture) et produit `world.book_read` avec le bénéfice configuré. Le bénéfice reste observationnel en V0.1 : son application aux croyances/mémoires dépend du futur moteur mémoire (hors U8). Les livres font partie du snapshot de reprise bit-à-bit et du snapshot d’observabilité quand activés. Accès spatial et durée de rédaction restent ouverts.
