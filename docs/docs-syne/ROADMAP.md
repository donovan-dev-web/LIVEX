...
| 7 | Ressources + Environnement | Saisons, régénération, obstacles | monde V2 — **cycle des ressources livré** (jalon U8, SYNE-070, engineVersion 0.7.0 : 4ᵉ type `Mineral`, régénération/dégradation périodique en fin de tick, persistance étendue) ; **constructions livrées** (SYNE-071, engineVersion 0.8.0 : obstacles statiques configurables `world.obstacleLayout[]`, mutation dynamique validée + tracée, grille A\* re-rasterisable — pas de mécanique agentique, mathématiques ouvertes) ; obstacles cercle V0.1 en place (ADR-013) ; **saisons (SYNE-072) restantes** |
